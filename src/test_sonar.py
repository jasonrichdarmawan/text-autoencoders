...
print(vec2text_model.model)
import inspect
print(inspect.getsource(vec2text_model.predict))

#   (decoder): ConditionalTransformerDecoderModel(
#     model_dim=1024
#     (decoder_frontend): TransformerEmbeddingFrontend(
#       model_dim=1024, no_scale=False
#       (embed): StandardEmbedding(num_embeddings=256206, embedding_dim=1024, pad_idx=1, init_fn=init_scaled_embedding)
#       (pos_encoder): SinusoidalPositionEncoder(encoding_dim=1024, max_seq_len=512)
#       (layer_norm): None
...
vec2text_model.predict(["Hello world"], source_lang="eng_Latn")
exit()
